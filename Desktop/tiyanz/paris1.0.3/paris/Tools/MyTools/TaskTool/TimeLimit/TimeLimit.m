...
- (void)openNet{
    NSString *bid = [[TimeLimit sharedTimeLimit] getTaskBid];
    NSString *user_id = [Const DataSGet:s_uid];
    if ([GB_ToolUtils isBlank:bid]||[GB_ToolUtils isBlank:user_id]) {
        return;
    }
    NSDictionary *dic  = [DeviceParam deviceParam];
    NSMutableDictionary *mut = [NSMutableDictionary dictionaryWithDictionary:dic];
    [mut setObject:user_id forKey:@"user_id"];
    [mut setObject:self.bid forKey:@"identifier"];
    [mut setObject:@(1) forKey:@"task_type"];
    NSString *url = [NSString stringWithFormat:@"%@%@",APIMain,@""];
    MBProgressHUD *hub = [MBProgressHUD bwm_showHUDAddedTo:[UIApplication sharedApplication].keyWindow title:nil];
    [NetRequestClass NetRequestNotEncryPOSTWithRequestURL:url WithParameter:mut WithHub: (MBProgressHUD *)hub WithReturnValeuBlock:^(NSDictionary *returnValue) {
        NSLog(@"task 已激活接口调用OK");
        // 后台已经记录了
        self.taskOpened = YES;
        self.timeHandle.timeInterval = [self.limit integerValue];
        if (self.startTime == 0) {
            NSDate *date = [NSDate dateWithTimeIntervalSinceNow:0];
            self.startTime = [NSString stringWithFormat:@"%f",[date timeIntervalSince1970]];
        }
    } WithErrorCodeBlock:^(NSDictionary *errorCode) {
        NSLog(@"ErrorCode error = %@",errorCode);
    } WithFailureBlock:^(NSString *errorMsg) {
        NSLog(@" Failure error = %@",errorMsg);
    }];
}

- (void)net_finishedTask{
    NSDictionary *dic  = [DeviceParam deviceParam];
    NSMutableDictionary *mut = [NSMutableDictionary dictionaryWithDictionary:dic];
    NSString *user_id = [Const DataSGet:s_uid];
    [mut setObject:user_id forKey:@"user_id"];
    [mut setObject:self.bid forKey:@"identifier"];
    [mut setObject:@(2) forKey:@"task_type"];
     NSString *url = [NSString stringWithFormat:@"%@%@",APIMain,@""];
    MBProgressHUD *hub = [MBProgressHUD bwm_showHUDAddedTo:[UIApplication sharedApplication].keyWindow title:nil];
    [NetRequestClass NetRequestNotEncryPOSTWithRequestURL:url WithParameter:mut WithHub: (MBProgressHUD *)hub WithReturnValeuBlock:^(NSDictionary *returnValue) {
        NSLog(@"task 倒计时结束，任务完成接口调用ok");
      NSLog(@"returnValue = %@",returnValue);
        [self taskClear];
    [[NSNotificationCenter defaultCenter] postNotificationName:g_taskMonitor object:@{@"taskOK":@"1"}];
        } WithErrorCodeBlock:^(NSDictionary *errorCode) {
        NSLog(@"ErrorCode error = %@",errorCode);
