...
- (void)configWeb{
    ...
    [self.userContentController addScriptMessageHandler:[[WeakScriptMessageDelegate alloc] initWithDelegate:self] name:js_music];
    [self.userContentController addScriptMessageHandler:[[WeakScriptMessageDelegate alloc] initWithDelegate:self] name:js_taskinfo];
    [self.userContentController addScriptMessageHandler:[[WeakScriptMessageDelegate alloc] initWithDelegate:self] name:js_clearTask];
    ...
    [self.userContentController addScriptMessageHandler:[[WeakScriptMessageDelegate alloc] initWithDelegate:self] name:js_service];//客服
    // 网络 相关
...
#pragma mark - 任务
- (void)js_music:(WKScriptMessage *)message{
    NSDictionary*param = message.body[@"param"];
    NSString *play = [NSString stringWithFormat:@"%@",param[@"play"]];
    if ([play isEqualToString:@"1"]) {
       [[MisicPlayer sharedMisicPlayer] play];
    }else if([play isEqualToString:@"0"]){
        [[MisicPlayer sharedMisicPlayer] pause];
    }
    [THEvaluate evaluateTip:@"home" WebView:self.webView method:message.body[@"callback"] dic:@{@"code":@(1)}];
}
- (void)js_startTask:(WKScriptMessage *)message{
    NSDictionary*param = message.body[@"param"];
    NSString *bid = param[@"bid"];
    NSString *limit = param[@"limit"];
    [[TimeLimit sharedTimeLimit] startTask:bid limit:limit];
    [THEvaluate evaluateTip:@"home" WebView:self.webView method:message.body[@"callback"] dic:@{@"code":@(1)}];
}
- (void)js_taskinfo:(WKScriptMessage *)message{ // type  0:未安装；1：打开目标app，返回YES；2：有任务正在进行中
    TimeLimit *lim = [TimeLimit sharedTimeLimit];
    NSString*bid = [GB_ToolUtils isNotBlank:lim.getTaskBid]?lim.getTaskBid:@"";
    NSString*downTime = [GB_ToolUtils isNotBlank:lim.getTaskCountDownTime]?lim.getTaskCountDownTime:@"";
    BOOL taskOK = lim.taskOK;
    [THEvaluate evaluateTip:@"home" WebView:self.webView method:message.body[@"callback"]
                        dic:@{@"bid":bid,
                              @"downTime":downTime,
                              @"taskOK":@(taskOK)
                              }];
}
- (void)js_clearTask:(WKScriptMessage *)message{
    [[TimeLimit sharedTimeLimit] taskClear];
    [THEvaluate evaluateTip:@"home" WebView:self.webView method:message.body[@"callback"] dic:@{@"code":@1}];
}

- (void)js_openApp:(WKScriptMessage *)message{
    NSDictionary *param = message.body[@"param"];
    NSString *bid = param[@"bid"];
    BOOL open = [Enc openAppWithNoEnc:bid];
    [THEvaluate evaluateTip:@"home" WebView:self.webView method:message.body[@"callback"] dic:@{@"code":@(open)}];
}
...
- (void)js_netFull:(WKScriptMessage *)message{
    ...
    [mut addEntriesFromDictionary:dic];
    if ([GB_ToolUtils isNotBlank:param]) {
...
- (void)netHome{
    ...
    [NetRequestClass NetRequestPOSTWithRequestURL:url WithParameter:dic WithHub: (MBProgressHUD *)hub WithReturnValeuBlock:^(NSDictionary *returnValue) {
        ...
        [hub bwm_hideWithTitle:@"网络解析错误" hideAfter:1.5];
    } WithFailureBlock:^(NSString *errorMsg) {
        [hub bwm_hideWithTitle:@"网络异常" hideAfter:1.5];
    }];
}
- (void)startApp{
...
- (void)js_netWithURL:(NSString *)url parameter:(NSDictionary *)parameter callback:(NSString *)callback{
    ...
    [NetRequestClass NetRequestPOSTWithRequestURL:url WithParameter:parameter WithReturnValeuBlock:^(NSDictionary *returnValue) {
        if (returnValue) {
        ...
        [THEvaluate evaluateTip:@"home" WebView:self.webView method:callback dic:@{@"code":@"-2",@"msg":@"网络解析错误"}];
    } WithFailureBlock:^(NSString *errorMsg) {
        [THEvaluate evaluateTip:@"home" WebView:self.webView method:callback dic:@{@"code":@"-1",@"msg":@"网络异常"}];
    }];
}
